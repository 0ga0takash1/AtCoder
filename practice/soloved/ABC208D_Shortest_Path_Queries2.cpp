...
int main() {
    in2(n, m);
    std::vector<std::vector<int64_t>> dis(n+1, std::vector<int64_t>(n+1, INF));
    rep(i, n) dis[i][i] = 0;
    ...
    int64_t ans = 0;
    rep(k, n) {
        rep(i, n) rep(j, n) chmin(dis[i][j], dis[i][k]+dis[k][j]);
        rep(i, n) rep(j, n) if ( dis[i][j] < INF ) ans += dis[i][j];
    }
    cout << ans << endl;
    return 0;
}
