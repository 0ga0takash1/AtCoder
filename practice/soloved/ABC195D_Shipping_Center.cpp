...
int main() {
    in3(n, m, q);
    vector<pair<int64_t, int64_t> > v_w(n);
    rep(i, v_w.size()) cin >> v_w[i].second >> v_w[i].first;
    Sort_pair(v_w, pair_Cf_R);
    ...
    rep(query, q) {
        ...
        vector<pair<int64_t, int64_t> > xx;
        rep(i, m) if ( i < l || r < i ) xx.push_back({x[i], 0});
        Sort_pair(xx, pair_Cf);
        // cout << query << endl;
        // repi(a, xx) cout << a.first << " " << a.second << endl;
        rep(i, v_w.size()) {
            rep(j, xx.size()) {
                int64_t *lug_val = &(v_w[i].first), *lug_size = &(v_w[i].second),
                        *box_val = &(xx[j].second), *box_size = &(xx[j].first);
                if ( *lug_size <= *box_size ) {
                    // -1があればそこにいれる
                    // -1がなければ該当する最小のboxに、すでに入っている荷物よりもvalがあれば入れる
                    int64_t ex0 = 0;
                    repb(k, j, xx.size()) {
                        if ( xx[k].second == 0 ) {
                            ex0 = 1;
