...
int main() {
    ...
    if ( s != s2 && s1 != s2 ) {
        ...
        rep(i, s.length()) set.insert(s[i]);
        rep(i, s1.length()) set.insert(s1[i]);
        rep(i, s2.length()) set.insert(s2[i]);

        if ( set.size() <= 10 ) {
            rep(i, pow(10, set.size()+1)) {
                string num = to_string(i);
                if ( num.length() < set.size() ) num.insert(0, "0");
                map<char, int64_t> map;
                int64_t index = 0;
                repi(a, set) {
                    map.insert({a, num[index]});
                    ++index;
                }
            }
        }
