...
int main() {
    vector<int64_t> area_candi;
    int64_t a = 1;
    while (1) {
        int64_t b = 1;
        int64_t area = 4*a*b+3*a+3*b;
        if ( area > 1000 ) break;
        while (1) {
            area = 4*a*b+3*a+3*b;
            if ( area > 1000 ) break;
            area_candi.push_back(area);
            ++b;
        }
        ++a;
    }
    // rep(i, area_candi.size()-1) {
    //     cout << area_candi[i] << " ";
    // }
    // cout << area_candi.back() << endl;
    ...
    rep(i, s.size()) {
        cin >> s[i];
        if ( !find_num(area_candi, s[i]) ) ++ans;
    }
    cout << ans << endl;
    return 0;
}
