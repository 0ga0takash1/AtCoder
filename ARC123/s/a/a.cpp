...
/*
    ...
    int64_t ans = INF;
    chmin(ans, 3*max(a, max(b, c))-a-b-c);
    if ( (a < b && b < c) || (a > b && b > c) ) {
        chmin(ans, max(b-a, c-b)-min(b-a, c-b));
    } else {
        int64_t num = abs(b-a)+abs(c-b);
        if (!(num%2)) {
            chmin(ans, (abs(b-a)+abs(c-b))/2);
        } else {
            chmin(ans, (abs(b-a)+abs(c-b))/2+2);
        }
    }
    cout << ans << endl;
*/
    int64_t num = 2*b-a-c, ans = 0;
    if ( num >= 0 ) {
        ans = num;
    } else {
        if ( !(num%2) ) {
