...
#define in3(n, m, k) long double n, m, k;cin >> n >> m >> k;
...
int main() {
    ...
    // rep_2(i, n-1) {
    rep(i, n) {
        // long double a = (h-hh[i])/(d-dd[i]), b = h-a*d;
        long double a = (h-p[i].second)/(d-p[i].first), b = h-a*d;
        int64_t can_see = 1;
        repb(j, i+1, n) {
            // cout << a*p[j].first+b << " " << p[j].second << endl;
            if ( p[j].second > a*p[j].first+b ) {
                can_see = 0;
                break;
            }
        }
        if (can_see) {
            // cout << p[i].first << " " << b << endl;
            chmin(ans, b);
            if (ans < 0) ans = 0.0;
            cout << ans << endl;
            return 0;
