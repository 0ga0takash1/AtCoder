...
#define vector2(a, n, k, num) std::vector<std::vector<int64_t>> a(n, std::vector<int64_t>(k, num))
#define ctoi(c) ((c)-'0')
...
int64_t pline(int64_t x, int64_t y) {
    int64_t left = max(1-a, 1-b), right = min(n-a, n-b);
    if ( a+left <= x && x <= a+right ) {
        if ( b+left <= y && y <= b+right ) {
            if ( y == x-a+b ) {
                return 1;
            }
        }
...
int64_t mline(int64_t x, int64_t y) {
    int64_t left = max(1-a, b-n), right = min(n-a, b-1);
    if ( a+left <= x && x <= a+right ) {
        if ( b-right <= y && y <= b-left ) {
            if ( y == a+b-x ) {
                return 1;
            }
        }
