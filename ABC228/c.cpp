...
int main() {
    ...
    rep(i, n) {
        in3(d1, d2, d3);
        point.push_back({d1+d2+d3, i});
    }
    Sort_rev(point);

    int64_t kth = point[k-1].first;
    vector<int64_t> ans(n);
    rep(i, point.size()) {
        if ( i >= k && point[i].first+300 < kth ) {
            ans[point[i].second] = 0;
        } else {
            ans[point[i].second] = 1;
        }
    }

    repi(a, ans) cout << ( (a)? "Yes" : "No" ) << endl;
    return 0;
}
