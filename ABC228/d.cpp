...
const int64_t MOD = 1<<20;

int64_t h_num ( vector<int64_t> v, int64_t search ) {
    if ( v.back() < search ) {
        return 0;
    }
...
int main() {
    vector<int64_t> a(MOD, -1), rem(MOD);
    iota(ALL(rem), 0);
    in1(q);
    rep(i, q) {
        in2(t, x);
        if ( t == 1 ) {
            int64_t h = h_num(rem, x);
            // while ( a[h%MOD] != -1 ) ++h;
            a[h] = x;
            rem.erase(rem.begin()+h);
        } else {
            cout << a[x%MOD] << endl;
        }
    }
